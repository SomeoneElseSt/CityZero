"""
Custom image pair matcher using COLMAP's matches_importer.

This script:
1. Ingests a .txt list of image pairs to compare
2. Uses COLMAP's matches_importer to feature match the custom pair list

The pair list can be generated by any method (spatial proximity, temporal sequences,
manual selection, etc.) as long as it follows COLMAP's pair format.
...
def feature_match(database_path, match_list_path) -> None:
    ...
    print("Feature matching using custom pair list")
    ...
    print("\nThis is expected, so do not quit the process\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Feature match images using COLMAP matches_importer with custom pair list")
    parser.add_argument(
        "--database_path",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to .txt file containing image pairs (one pair per line: image1.jpg image2.jpg)"
    )
